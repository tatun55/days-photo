...
class EpsonConnectApiService
{
    public function printCartItem($cartItem)
    {
        ...
        foreach ($cartItem->photos()->get() as $key => $photo) {

            $content_type = 'application/octet-stream';

            $key++;
            $file_path = \Storage::disk('s3')->url("{$photo->album_id}/{$photo->id}/o.jpg");
            $upload_uri = $base_uri . '&File=' . $key . '.jpg';
            // dd($file_path, $upload_uri);
